    ...
    sidebarPanel(
      ...
      fluidRow(
        sliderInput("beta", "Flexible Beta:",
                    min = 0, max = 1, value = 0.625, step= 0.001)),
...
server <- function(input, output, session) {
  ...
  plotInput <- reactive({
    ...
    demoflex <- agnes(bc,method='flexible',par.method=input$beta)
    ...
    #ordispider(bci.mds, input$`Variable 2`, col="skyblue")
    ordispider(bci.mds, spe.bw.groups, col="blue", label=TRUE)
    ordiellipse(bci.mds, spe.bw.groups, col="red")
